...
class ProductsApiApplicationTests {
	...
	public void testGetAll() throws Exception {
		...
		List<ProductResponseDto> productList = Arrays.asList(response1, response2);
		
		when(productDomainService.getAll(any())).thenReturn(productList);
		...
		mockMvc.perform(get("/api/products")
				...
				.andExpect(jsonPath("$[0].quantity").value(response1.getQuantity()))
				.andExpect(jsonPath("$[1].name").value(response2.getName()))
				.andExpect(jsonPath("$[1].price").value(response2.getPrice()))
				.andExpect(jsonPath("$[1].quantity").value(response2.getQuantity()));
	}
	...
	public void testGetById() throws Exception {
		...
		when(productDomainService.getById(1L)).thenReturn(response);
